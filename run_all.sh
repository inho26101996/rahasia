#!/bin/bash
	...
	if [ ! -f ".venv_installed" ]; then
	    ...
	    while IFS= read -r package; do
	        echo "$(date) - Mencoba menginstal $package..." >> log.txt
	        pip install --ignore-installed --no-deps "$package" || echo "$(date) - Gagal menginstal $package. Melanjutkan..." >> log.txt
	    done < requirements.txt
	    touch .venv_installed
	    echo "$(date) - Proses instalasi dependensi Python di dalam venv selesai (beberapa mungkin gagal)." >> log.txt
	fi
